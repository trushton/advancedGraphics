...
bool shape::createMesh(OGRSpatialReference* sr2, glm::vec2 origin, glm::vec2 scale, Terrain& t)
{
    ...
    vector<TerrainVertex> vertexs = {};
    vector<int> indicies = {};
    Renderer.init();

    cout << "LINE RENDERER CREATOR" << endl;
    if (!Renderer.addShader(GL_VERTEX_SHADER, "../shaders/linevert.shader"))
    {
        cout << "Failed to add shader" << endl;
        return false;
    }
    if (!Renderer.addShader(GL_FRAGMENT_SHADER, "../shaders/linefrag.shader"))
    {
        cout << "Failed to add shader" << endl;
        return false;
    }
    ...
    for (uint i = 0; i < points.size(); i++)
    {
        ...
        for (uint j = 0; j < points[i].size(); j++)
        {
            ...
            TerrainVertex temp = {{(float)x, (float)t.SampleTerrain(orig), (float)y}, {(float)1, (float)0, (float)0}, {(float)1, (float)1}};
            ...
            if (j + 1 < points[i].size())
            {
                ...
                TerrainVertex temp2 = {{(float)x, (float)t.SampleTerrain(orig), (float)y}, {(float)1, (float)0, (float)0}, {1, 1}};
    ...
    vbo_geometry.allocateBufferData(sizeof(TerrainVertex)*vertexs.size(), &vertexs[0], GL_STATIC_DRAW);
...
void shape::render(glm::mat4& view, glm::mat4& projection)
{
    ...
    Renderer.setGLVertexAttribPointer("v_position", 3, GL_FLOAT, GL_FALSE, sizeof(TerrainVertex), 0);
