...
void DeferredShading::tick(float dt)
{
...
void DeferredShading::DSGeometryPass()
{
...
void DeferredShading::DSStencilPass(unsigned int PointLightIndex)
{
    m_nullTech.enable();
...
/*void DeferredShading::BeginLightPasses()
    {
        glEnable(GL_BLEND);
...
void DeferredShading::DSPointLightsPass(unsigned int PointLightIndex)
{
...
float DeferredShading::CalcPointLightBSphere(const PointLight &Light)
{
...
/*
        ...
        float returnVal = sqrtf((maxColor * 256 * Light.DiffuseIntensity / Light.Attenuation.Exp) + Light.Attenuation.Constant);
        return returnVal;*/
}

float DeferredShading::CalcSpotLightBSphere(const SpotLight &Light)
{
    float MaxChannel = fmax(fmax(Light.Color.x, Light.Color.y), Light.Color.z);
...
void DeferredShading::DSDirectionalLightPass()
{
    m_gbuffer.BindForLightPass();
...
void DeferredShading::InitBoxPositions()
{
...
/*void DeferredShading::DSLightPass()
    {
