...
void createMesh(vector<vector<float>>& input, float xres, float yres, float max, vector<int>& indicies, vector<TerrainVertex>& vertexes)
{
cout << "MAX: " << max;
cout << "XRES " << xres << " YRES" << yres << endl;
// Our vertex information along with normals contained inside
std::vector<TerrainVertex> vectors = vector<TerrainVertex>();
